...
\[
\Pi(G;R) = \bigl\{\, \{ S \in R : f(G,S)=\sigma \} \;\big|\; \sigma \in f(G,R) \,\bigr\}.
\]
...
=
\begin{cases}
\text{the unique element of } R & \text{if } |R|=1 \\[6pt]
\displaystyle
\arg\min_{G \in R}
\begin{cases}
\bigl(W(G;R),\; E(G;R),\; \mathrm{name}(G)\bigr) & \text{minimax strategy} \\[4pt]
\bigl(E(G;R),\; W(G;R),\; \mathrm{name}(G)\bigr) & \text{expected strategy}
\end{cases}
\end{cases}
\]

Lexicographic order on the triple; \(\mathrm{name}(G)\) breaks remaining ties alphabetically.
